...
// URL and key come from the shared config — do not read import.meta.env here.
import { SUPABASE_URL as supabaseUrl, SUPABASE_ANON_KEY as supabaseAnonKey } from './config';
...
export async function callQueueManager(params: {
  ...
  try {
    ...
    // Unwrap ApiResponse envelope { success, data } → return inner data
    // Falls back to raw shape for backwards compatibility
    if (data && typeof data === 'object' && 'success' in data) {
      if (!data.success) {
        throw new Error(data.error || 'Queue manager error');
      }
      return data.data;
    }
...
export async function callPlayerControl(params: {
  ...
  if (data && typeof data === 'object' && 'success' in data) {
    if (!data.success) throw new Error(data.error || 'Player control error');
    return data.data;
  }
  return data;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        // Handle both envelope { success: false, error } and legacy { error } shapes
    ...
    // Unwrap ApiResponse envelope { success, data } → return inner data
    if (data && typeof data === 'object' && 'success' in data) {
      if (!data.success) throw new Error(data.error || 'Kiosk handler error');
      return data.data;
    }
    return data;
  } catch (err: any) {
...
export async function callPlaylistManager(params: {
  ...
  // Unwrap ApiResponse envelope
  if (data && typeof data === 'object' && 'success' in data) {
    if (!data.success) throw new Error(data.error || 'Playlist manager error');
    return data.data;
  }
  return data;
}
